...
class Mark {
   ...
   * @access protected
   */
  static matchesExclude(el) {
    return DOMIterator.matches(el, ['.ignore'].concat([
  ...
  wrapRangeInTextNode(node, start, end) {
    ...
    // todo 进行节点包裹
    console.log('节点包裹 wrapRangeInTextNode', repl.textContent, {node, start, end, repl, startNode});
    return ret;
  }
  ...
  wrapRangeInMappedTextNode(dict, start, end, filterCb, eachCb) {
    ...
    dict.nodes.every((n, i) => {
      ...
      if (typeof sibl === 'undefined' || sibl.start > start) {
        ...
        console.log('wrapRangeInMappedTextNode', {s, e, startStr, endStr}, dict.value);
  ...
  wrapRangeFromIndex(ranges, filterCb, eachCb, endCb) {
    console.log('wrapRangeFromIndex.ranges', ranges);
    this.getTextNodes(dict => {
      const originalLength = dict.value.length;
      console.log(`获取所有节点和所有字符串=`, dict, `需要标记的位置=`, ranges);
      ranges.forEach((range, counter) => {
        ...
        console.log('对选中的字符串进行标记', {start, end, valid}, dict.nodes);
        if (valid) {
          this.wrapRangeInMappedTextNode(dict, start, end, node => {
            return filterCb(
              node,
  ...
  unmark(opt) {
    ...
    this.iterator.forEachNode(NodeFilter.SHOW_ELEMENT, node => {
      ...
      const matchesSel = DOMIterator.matches(node, sel),
        matchesExclude = Mark.matchesExclude(node);
